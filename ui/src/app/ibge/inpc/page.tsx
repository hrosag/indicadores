...
export default function Page() {
  const [rows, setRows] = useState<InpcRow[]>([]);
  ...
  const loadData = async (options?: {
    ...
    try {
      ...
      const data = await fetchInpcMonthly({ start: startValue, end: endValue, auto });
      setRows(data);
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        const range = await fetchInpcMinMaxDate();
  ...
  const handleExport = async () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "INPC");
