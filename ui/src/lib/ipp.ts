...
export type IppRow = IndicatorRowBase & {
  ano: number | null;
  mes: string | null;
...
export const parseRow = (row: Record<string, string | number | null>): IppRow => ({
...
export const getMinMaxDate = (rows: IppRow[]) => {
...
export const fetchIppMinMaxDate = async () => {
  const qMin = supabase
    .from("vw_ipp_6904_monthly")
    .select("data")
...
export const fetchIppMonthly = async ({ start, end, auto }: FetchParams) => {
  let query = supabase
    .from("vw_ipp_6904_monthly")
    .select("*")
    .order("data", { ascending: true });
